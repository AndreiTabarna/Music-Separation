...
for idx in range(len(train_dataset)):
    ...
    drum_spec = output_tensors['drums'].numpy().real
...
for idx in range(len(test_dataset)):
    _, output_tensors = test_dataset[idx]
    drum_spec = output_tensors['drums'].numpy().real
    original_spec = np.load(test_dataset.samples[idx][0]).real
    drum_ratio = drum_spec / (original_spec + 1e-8)
    threshold = 0.1
    drum_mask = (drum_ratio > threshold).astype(np.float32)
    drum_masks.append(drum_mask)
...
np.save('general_drum_mask.npy', general_drum_mask)

# Test the general drum mask on one instance
# Load an example song
example_song_idx = 44
input_path, output_paths = test_dataset.samples[example_song_idx]
...
temp_wav_file = "general_masked_audio.wav"
